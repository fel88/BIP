...
void setMyFont(){
  if(textSize==0){
    ...
    stext1.setFreeFont(&exportFont);   
       ...
       stext1.setFreeFont(&exportFont2); 
    tft.setFreeFont(&exportFont2);}
    else{
        stext1.setFreeFont(&exportFont3); 
    tft.setFreeFont(&exportFont3);
      }
}
void setMyFont1(){
...
tft.setRotation(1);
  ...
  //stext1= &TFT_eSprite(&tft); // Sprite object stext1
       ...
       #ifdef   SCREEN_128
         stext1.createSprite(128,128+32);
         stext1.setScrollRect(0, 0, 128,128+32, TFT_BLACK);     // here we set scroll gap fill color to blue
       #else
         stext1.createSprite(160,128+32);
         stext1.setScrollRect(0, 0, 160,128+32, TFT_BLACK);     // here we set scroll gap fill color to blue
...
{ String button; button = "NONE";  
  ...
  int i, z, sum;
 //Serial.println(digitalRead(key2));
...
if(smoothScroll)
   stext1.pushSprite(0, 32);
   else
  stext1.pushSprite(0, 0);
    

      nextWordRequired=true;
       if(line1.length()>0 && (line1Changed || smoothScroll)){
  ...
  //stext1.setTextColor(TFT_GREEN, TFT_BLUE);
     ...
     int ypos=25+yShift2+height*5;
                 if(textReadMode==1){
      ypos=25+yShift2+height*3;}
  //stext1.println(data);
  ...
  if(smoothScroll){
    ...
    #ifdef   SCREEN_128
      stext1.setCursor(64-padding,tft.height());
        #else
          stext1.setCursor(tft.width()/2-padding,tft.height());
          #endif
  ...
  if(scrollAccum>=stext1.fontHeight(1)){
    scrollAccum=0;
    ...
    nextWordRequired=true;
      ...
      #ifdef   SCREEN_128
      stext1.setCursor(64-padding,ypos);
        #else
          stext1.setCursor(tft.width()/2-padding,ypos);
          #endif
...
void loop() {
  ...
  bookBtnsUpd();
      ...
      if(textReadMode==0 || (textReadMode==1 && useStaticDelay))
    {
      speedSum=speed;
      if(smoothScroll){
        speedSum=speed/10;
  ...
  //textReadMode=0 - scroll
   //if(textReadMode==0 && scrollAccum!=0)return;
  if(!pauseBook || (nextPageGo && textReadMode==0)){
   ...
   if(textReadMode==1 ||/* yShift>11 */ (textReadMode==0 && nextWordRequired) || (textReadMode==2 && nextPageGo) /*|| xShift>=(cw)*/){
    yShift=0;
    nextWordRequired=false;
    int repeats=1;
